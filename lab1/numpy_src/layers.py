...
class Conv2D(Layer):
    ...
    # def forward(self, x):
    #     self.x = x
    #     weights = self._params['weights']['value'].reshape(self.out_channels, -1)  # o,ckk
    #
    ...
    #                  (h + self.padding * 2 - self.ksize) // self.stride + 1,
    #                  (w + self.padding * 2 - self.ksize) // self.stride + 1)
    #
    #     # self.col_img = self.im2col(x, self.ksize, self.stride)  # bhw * ckk
    #     self.col_img = im2col_cython(
    #         x,
    #         self.ksize, self.ksize,
    #         self.padding,
    #         self.stride
    #     )
    #     out = np.dot(weights, self.col_img).reshape(self.out_channels, b, -1).transpose(1, 0, 2)
    #
    #     self.out = np.reshape(out, out_shape)
    #     for i in range(self.out.shape[1]):
    #         self.out[:, i, :, :] = self.out[:, i, :, :] + self._params['bias']['value'][i]
    #     return self.out
    #
    # def backward(self, grad_out):
    ...
    #     del grad_out_
    #
    #     self._params['weights']['grad'] = np.dot(grad_out_flat, self.col_img.T).reshape(
    #         self._params['weights']['grad'].shape)
    #     self._params['bias']['grad'] = np.sum(grad_out_flat, axis=1)
    #     # tmp = self.ksize - self.padding - 1
    #     # grad_out_pad = np.pad(grad_out, ((0, 0), (0, 0), (tmp, tmp), (tmp, tmp)), 'constant', constant_values=0)
    #     #
    #     # flip_weights = np.flip(self._params['weights']['value'], (2, 3))
    ...
    #
    #     weights = self._params['weights']['value'].transpose(1, 0, 2, 3).reshape(self.in_channels, -1)
    #
    #     # col_grad = self.im2col(grad_out_pad, self.ksize, 1)  # bhw,ckk
    #     col_grad = im2col_cython(
    #         grad_out,
    #         self.ksize, self.ksize,
    #         self.padding,
    #         self.stride
    ...
    #
    #     # (in,ckk) * (bhw,ckk).T
    #     next_eta = np.dot(weights, col_grad).reshape(self.in_channels, b, -1).transpose(1, 0, 2)
    #
    #     try:
